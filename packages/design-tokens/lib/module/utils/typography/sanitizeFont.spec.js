...
describe('sanitizeFontFaces', () => {
  ...
  it('Several, properly quoted', () => {
    expect(sanitizeFontFamily("'Open Sans','Noto Sans'")).toEqual("'Open Sans', 'Noto Sans'");
  });
  it('Several, improperly quoted', () => {
    expect(sanitizeFontFamily("'Open Sans, Noto Sans'")).toEqual("'Open Sans', 'Noto Sans'");
  });
  it('Several, mixed properly & improperly quoted', () => {
    expect(sanitizeFontFamily("Helvetica neue, 'sans-serif'")).toEqual("'Helvetica neue', sans-serif");
  });
  it('Kitchen sink', () => {
    expect(sanitizeFontFamily('fixed,Helvetica neue, " Comic Sans " , sans-serif')).toEqual("fixed, 'Helvetica neue', 'Comic Sans', sans-serif");
  });
});
