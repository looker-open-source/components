...
`@looker/components` expects two prequesites to exist for the complete styling to be applied:

### GlobalStyle

At the top-most level of the usage of `@looker/components` you need to load the `<GlobalStyle />` component this will set a simple CSS reset as well as basic font assignment. [GlobalStyle documentation &rarr;](/utilities/globalstyle)

### Font

Users may have the `@looker/components` default font, Open Sans, installed on their computers so we recommend importing the font to ensure that content is rendered with the proper font-face.
