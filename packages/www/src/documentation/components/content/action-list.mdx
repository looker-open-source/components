...
;() => {
  ...
  const MyActions = () => (
    <>
      <ActionListItemAction onClick={() => alert(`Cheese selected!`)}>
        Select Cheese
...
<Props of="ActionListLayout" />

In the event that your page and `<ActionList>` render before data is available (i.e. before a Promise resolves), you can use an `<ActionListLayout>` component to conditionally render a loading icon until your data is ready.

Passing a falsy value into an `<ActionListLayout>`'s `isLoading` prop will render a `<Spinner>` icon. Once the `isLoading` prop receives a truthy value, its child will rendered instead.

You can also use an `<ActionListLayout>` element to render a "No Results" message using the `noResults` prop.

<i>Example coming soon...</i>
