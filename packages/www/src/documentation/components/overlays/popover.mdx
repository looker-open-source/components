...
Popovers are containers for things like contextual information, menus, and related actions. They are displayed when a user's action triggers them,
usually a button click.

## Terminology

Popovers are assembled of two pieces an overlay and a surface. Unlike [Modals]('/modals'), the `Popover` overlay is transparent, but it still blocks the application behind it until it is closed, with the exception of [grouped Popovers]('#grouped-popovers'). The `Popover`'s' sufrace renders the content above the overlay and the rest of the application.
