...
The `<FieldTextArea />` component is composed of an `<TextArea />` component and a `<Label />` component. Using `<FieldTextArea />` allows for rendering validation messages.

```jsx
<FieldTextArea label="Tnis is the label" />
```
