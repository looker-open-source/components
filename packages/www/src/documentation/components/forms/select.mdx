...
To customize this, use `indicator` prop, which accepts a React node (including `false` to remove it
completely) or a function accepting `OptionStatuses` and returning a React node.
