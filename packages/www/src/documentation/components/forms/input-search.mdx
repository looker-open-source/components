...
The `<InputSearch />` component renders a single search input element. You can add a placeholder, and a starter value, also a summary text and a closing button associated with it. To remove the button and summary use the hideControls option. If you don't want to see the search `Icon` use the prop `hideSearchIcon`
