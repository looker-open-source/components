...
var getDateFilterTokenItem = function getDateFilterTokenItem(item, configType) {
  switch (configType) {
    ...
    case 'day_range_picker':
      {
        var dateItem = item.date || (0, _filterExpressions.dateToFilterDateTimeModel)(new Date(Date.now()));
        var _item$start4 = item.start,
          start = _item$start4 === void 0 ? dateItem : _item$start4,
          _item$end4 = item.end,
          end = _item$end4 === void 0 ? (0, _filterExpressions.dateToFilterDateTimeModel)((0, _filterExpressions.addDays)(new Date(Date.now()), 1)) : _item$end4;
        return (0, _filterExpressions.sanitizeDate)(_objectSpread(_objectSpread({}, item), {}, {
          type: 'range',
          start: start,
          end: end
