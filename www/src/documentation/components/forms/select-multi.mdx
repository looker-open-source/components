...
by the enter key, comma, or tab or newline character when pasting. This works best when the option label is the
same as the value. This prop also supports accepting values copy-pasted in from another `SelectMulti`.
