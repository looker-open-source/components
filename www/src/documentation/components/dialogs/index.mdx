...
Modals come in two variants: [`Dialog`](/components/modals/dialog) which overlays the entire screen.
...
Using the `DialogContent` component quickly gives your content consistent spacing inside a modal component as well as managing overflow of content of the dialog is taller than the viewport.
...
<Props of="DialogHeader" />

`DialogHeader` is attached to the top of modal and provides a button to close the containing modal. Uses the `DialogContext` to get access to the `closeModal` callback.
...
`DialogHeader` also accepts an optional `headerIcon` prop which can be used to communicate context or intent of the modal. For example: if you need the user to confirm a destructive action, it can be helpful to render an eyecatching warning icon next to the title text:

```jsx
<DialogHeader headerIcon={<Icon name="Warning" color="critical" size={22} />}>
...
`DialogContext` is a [React Context](https://reactjs.org/docs/context.html) that provides access to functionality without requiring the developer to explicitly manage the Modal's state. The context provides access to the following methods:
