---
title: Dialog Layout
---

import { Props } from '../../../Shared'

## DialogContent

<Props of="DialogContent" />

Using the `DialogContent` component quickly gives your content consistent spacing inside a dialog component as well as managing overflow of content of the dialog is taller than the viewport.

DialogContent supports all [`Box`](/src-documentation-components-box) properties.

```jsx
<DialogContent>
  <Box height="2rem" bg="rebeccapurple" />
</DialogContent>
```

If content overflows the component's height an visual indicator of content that extends beyond the visible edges is added (border on top and shadow on the bottom).

```jsx
<Box height="10rem" display="flex" bg="white" p="large">
  <DialogContent>
    <Paragraph>Scroll down here...</Paragraph>
    <Box height="6rem" bg="rebeccapurple" />
  </DialogContent>
</Box>
```

## DialogHeader

<Props of="DialogHeader" />

`DialogHeader` is attached to the top of dialog and provides a button to close the containing Dialog. Uses the `DialogContext` to get access to the `closeModal` callback.

DialogHeader supports all [`Box`](/src-documentation-components-box) properties.

```jsx
<DialogHeader>This is a DialogHeader</DialogHeader>
```

## DialogFooter

<Props of="DialogFooter" />

`DialogFooter` is attached to the bottom of dialog. Generally, the terminal action for a dialog as well as an option to abandon the process done via the dialog are available within this component. This component is generally used in combination with the `DialogContext.Provider` to get access to the `DialogContext.closeModal()` method.

DialogFooter supports all [`Box`](/src-documentation-components-box) properties.

```jsx
<DialogFooter>
  <Button>Save Changes</Button>
</DialogFooter>
<Divider />
<DialogFooter>
  <Button>Save Changes</Button>
  <ButtonTransparent>Cancel</ButtonTransparent>
</DialogFooter>
<Divider />
<DialogFooter
  secondary={
    <ButtonOutline iconBefore="Plus">
      Create Item
    </ButtonOutline>
  }
>
  <Button>Save Changes</Button>
  <ButtonTransparent>Cancel</ButtonTransparent>
</DialogFooter>
<Divider />
<DialogFooter
  secondary={
    <React.Fragment>
      <ButtonOutline iconBefore="Plus">
        Create Item
      </ButtonOutline>
      <ButtonOutline iconBefore="Plus">
        Yet Another
      </ButtonOutline>
    </React.Fragment>
  }
>
  <Button>Save Changes</Button>
  <ButtonTransparent>Cancel</ButtonTransparent>
</DialogFooter>
<Divider />
<DialogContent>
  <Box bg="lavender" p="medium">
    Example content
  </Box>
</DialogContent>
<DialogFooter>
  <Button size="xsmall">Save Changes</Button>
  <ButtonTransparent size="xsmall">
    Cancel
  </ButtonTransparent>
</DialogFooter>
```
