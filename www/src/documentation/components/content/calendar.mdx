...
import { MessageBar, Code } from '@looker/components'
import { CalendarPropTable } from './demos'

<MessageBar my="large" canDismiss={false}>
  Note: Calendar must be imported from its specific location rather than the
  package root.
</MessageBar>
...
<MessageBar>
  If you need an interactive date picker, use{' '}
