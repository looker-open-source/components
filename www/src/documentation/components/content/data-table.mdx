...
For default sort behavior, use the `doDefaultDataTableSort` helper function. The `doDefaultDataTableSort` helper function will return a sorted data array as well as an updated columns array, which can then be used to replace your existing arrays post-sort.

**Note:** `doDefaultDataTableSort` currently supports `string`, `number`, and `date` sorting. It will use the `type` property on the corresponding column object of the passed-in `id` to determine which sort logic to use.
