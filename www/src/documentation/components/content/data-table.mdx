...
The `<DataTable>` component, like `<Table>`, displays tabular data. However, unlike a `<Table>`, an `<DataTable>` allows you to bake in a set of actions on each row. Common use cases include:
...
<Props of="DataTableItem" />

The `<DataTableItem>` component represents a row of data in your Action List.
...
<Props of="DataTableCell" />

The `<DataTableCell>` component is equivalent to a data cell in your Action List. `<DataTableCell>` elements should always be children of an `<DataTableItem>` element (i.e. each row should contain some number of column elements).

In its simplest form, an `<DataTableCell>` element will display whatever child elements it has.
  ...
  return (
    <DataTable columns={columns}>
      <DataTableItem id={'cheddar'} onClick={() => alert(`Row clicked`)}>
        <DataTableCell detail={link} indicator={indicator}>
          Cheddar
        </DataTableCell>
      </DataTableItem>
    </DataTable>
...
**Note:** By default, clicking on any element within an `<DataTableCell>` will trigger the `onClick` of the parent `<DataTableItem>`. To prevent this, make sure to include an `event.stopPropagation()` call within your `<DataTableCell>`'s `onClick`.

## DataTableAction

<Props of="DataTableAction" />

The `<DataTableAction>` component represents an actionable item on the rows of your Action List. As noted above, you can supply each of your rows with actions by passing `<DataTableAction>` elements into the `actions` prop of the parent `<DataTableItem>` element.

## ActionListManager

<Props of="ActionListManager" />

In the event that your page and `<DataTable>` render before data is available (i.e. before a Promise resolves), you can use an `<ActionListManager>` component to conditionally render a loading icon until your data is ready.
...
You can implement sorting on an `<DataTable>` by passing a function into the optional `onSort` prop.
...
You can add row select behavior on an `<DataTable />` by passing a valid into the `select` prop.
...
### useDataTableSelectManager

The `useDataTableSelectManager` hook can be used to quickly create the standard building blocks of `DataTable` select behavior.

`useDataTableSelectManager` accepts the following parameters:

<SelectManagerParameterTable />

`useDataTableSelectManager` returns an object with the following properties:
...
;() => {
  ...
  const {
    ...
    selections,
    setSelections,
  } = useDataTableSelectManager(allSelectableItems)
...
An `DataTable` will reveal its control bar when the following conditions are met:

1. The array passed into the `select.selectedItems` prop has length > 0.
2. A valid object is passed into the `bulk` prop.

The `bulk` prop is used to configure an `DataTable`'s control bar. The object passed into `bulk` _must_ have the following properties:
...
**Note:** The `onSelect` returned from the `useDataTableSelectManager` hook is designed to work seamlessly with the control bar. In the event that a user selects all items, both visible and non-visible, a follow-up click on a checkbox will unselect both the clicked item and all non-visible items.
...
;() => {
  ...
  const {
    ...
    selections,
    setSelections,
  } = useDataTableSelectManager(pageItemIds)
