...
Use the `icon` prop to display an icon left of the `Tree` label.
...
Use the `detail` prop to display an element right of the `Tree` label.

**Note:** Any elements passed into the `detail` prop will not propagate events (i.e. clicking on a `detail` element will not open or close the `Tree`).
...
Use the `detailAccessory` prop if you do not want to apply the hover background onto the `detail` elements.
