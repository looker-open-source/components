...
If your `Tree` has an optional `icon`, set `forceLabelPadding` to `true`. If no `icon` prop, set `forceLabelPadding` to `"no-icon"`.
...
<TreeCollection>
  ...
  <Tree label="Cheeses" defaultOpen forceLabelPadding="no-icon">
    <TreeItem>Gouda</TreeItem>
    <TreeItem>Swiss</TreeItem>
    <TreeItem>Cheddar</TreeItem>
  </Tree>
