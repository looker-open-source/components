...
Because of the layout changes from `labelBackgroundOnly`, a separate callback must be assigned to the indent spacing of child `TreeItem`s. Use `TreeItem`'s `onClickWhitespace` prop to assign a callback to the indent padding of a given item.

**Note:** When `labelBackgroundOnly` is true, `TreeItem`s should use `itemRole="none"`.

```jsx
<Tree label="Cheeses" labelBackgroundOnly defaultOpen>
  <TreeItem itemRole="none" onClickWhitespace={() => alert('Gouda time.')}>
    <span onClick={() => alert('Gouda time.')}>Gouda</span>
  </TreeItem>
  <TreeItem
    itemRole="none"
    ...
    selected
  >
    <span onClick={() => alert('Swiss time.')}>Swiss</span>
  </TreeItem>
  <TreeItem itemRole="none" onClickWhitespace={() => alert('Cheddar time.')}>
    <span onClick={() => alert('Cheddar time.')}>Cheddar</span>
  </TreeItem>
</Tree>
```
