...
Use the `noIndent` prop to prevent your nested `Tree` from being indented so it's label is vertically aligned with adjacent `TreeItem`s.
...
<Tree label="Orders" icon="Table" defaultOpen>
  <Tree visuallyAsBranch noIndent label="Created" defaultOpen>
