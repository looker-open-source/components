...
`TreeCollection` acts as a wrapper component to `Tree` and `TreeItem` elements. For accessibility purposes, all `Tree` and `TreeItem` compositions should have a `TreeCollection` at the top.
