...
- `itemsSelected`: An array containing the `id`s of all selected items. Will usually be a piece of state.

- `itemsVisible`: An array containing the `id`s of all currently visible items. This is mainly used to help calculate the checked state of the header checkbox.
    ...
    <>
      <ActionList
        ...
        select={{
          ...
          itemsVisible: allSelectableItems,
...
1. The array passed into the `select.itemsSelected` prop has length > 0.
