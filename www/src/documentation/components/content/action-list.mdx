...
1. The array passed into the `itemsSelected` prop has length > 0.
...
- `pageCount`: The total number of items, both visible and nonvisible, in this `ActionList`
...
;() => {
  ...
  const onTotalSelectAll = () =>
    setSelections([...allSelectableItems, 'Swiss', 'Pepper Jack', 'American'])
