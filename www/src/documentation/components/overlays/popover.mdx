...
As an avaliable mobile optimization, we provide the `renderMobileDialog` prop. Setting this prop will render a [full-screen Dialog](/components/dialogs/) on screen sizes less than 480px wide. The associated `label` prop will rendered in the dialog header.
